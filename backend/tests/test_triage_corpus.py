...
def _clean(tmp_path, monkeypatch):
    ...
    offline_store = triage.TriageStore(allow_ledger_only=True)
    monkeypatch.setattr(triage, "store", offline_store)
    yield
    offline_store.clear()
        ...
        triage.store.observe(
            Event.wrap(
                IncidentUpdate(
                    ...
                    title="t", anomaly_type="vandalizm", risk="orta",
                ),
                feed=f"kamera{n}",
            )
        )
...
def test_offline_feedback_never_auto_dismisses_without_rule_approval(tmp_path):
    triage.store.observe(run_started())
    ...
    pending = triage.store.snapshot()["pending"]
    assert [item["incident_id"] for item in pending] == ["INC-99"]
    assert pending[0]["signals"]["durum_p"] == pytest.approx(0.33)
    assert triage.store.auto_dismissed == 0
...
def test_decide_endpoint_carries_operator_correction_to_the_ledger(tmp_path):
    ...
    with TestClient(app) as client:
        r = client.post("/api/triage/decide", json={
            ...
            "reviewer": "bengisu", "risk_level": "orta",
            "start_time": 10.0, "peak_time": 12.0, "end_time": 18.5,
